...
use <Prusa_MK3S+_X-Axis-end_for_SLS.scad>

module x_end_idler_holes(){
    x_end_holes();
    translate([0,3.5,0]){
        // Pulley screw
        translate([0,-19,30.25]) rotate([0,-90,0]) cylinder(h=80,r=1.55,$fn=50);
        translate([-4,-19,30.25]) rotate([0,-90,0]) cylinder(h=4,r=3,$fn=50);

        translate([-21.5,-19,30.25]) rotate([0,-90,0]){
            rotate([0,0,30]) cylinder(h=4,r=3.1,$fn=6);
        }

        translate([-24,-19,30.25]) rotate([0,-90,0]){
            rotate([0,0,30]) cylinder(h=4,r1=3.1,r2=7,$fn=6);
        }

        translate([-4.5,-19,30.25]) rotate([0,-90,0]) cylinder(h=1,r1=4,r2=3,$fn=50);

        // Pulley side cut
        translate([-9,-19,30.25]) rotate([0,-90,0]) cylinder(h=9,r=9.5,$fn=30);
...
module x_end_idler_base(){
    difference(){
        union(){
            x_end_base();
            difference(){
               translate([-6.5,-21,13.5]) cube([1,12.5,42]);
               translate([-24,-11.5,19]) cube([20,10,24]);
               translate([-6.5,-10,55.5]) rotate([180,-45,0]) cube([1.5,19,5]);
               translate([-6.5,-29,13.5]) rotate([0,45,0]) cube([1.5,19,5]);
               translate([-5.5,-19,13.5]) rotate([90,0,-35]) cube([1.5,45,6]);
            }
            // Pulley nut support
            difference(){
               translate([-23.5,-15.5,30.25]) rotate([0,-90,0]) rotate([0,0,30]) cylinder(h=1,r1=5.5,r2=3.7,$fn=30);
    ...
    difference(){
        translate([-19,-16.5,30.25]) rotate([0,-90,0]) cylinder(h=1,r1=9,r2=12,$fn=30);
        translate([-25,-11.5,19]) cube([20,10,24]);
        translate([0,-15.5,30.25]) rotate([0,-90,0]) cylinder(h=80,r=1.55,$fn=30);
    }
...
module x_end_idler(){
    mirror([0,1,0]) difference(){
        ...
        translate([-8,-15.5,30.25]) rotate([0,-90,0]) cylinder(h=20,r=1.55,$fn=30);
